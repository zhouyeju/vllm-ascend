...
def _get_npu_num_per_ray_node():
    npu_nums = []
    for e in ray.nodes():
        num = e.get("Resources", {}).get("NPU", None)
        if num:
            npu_nums.append(int(num))
    return max(npu_nums)


def split_dp_resources(tp_size: int,
                       dp_size: int,
                       npu_pack_max_size: int = 8) -> List[int]:
    """
    ...
    """
    assert tp_size <= npu_pack_max_size, (
        f"do not allow TP size to exceed the number of NPUs on a single node {npu_pack_max_size}"
    )
    total_npu = dp_size * tp_size
    group_size = (npu_pack_max_size - (npu_pack_max_size % tp_size)
                  if npu_pack_max_size % tp_size != 0 else npu_pack_max_size)
...
async def make_dp_group(pd_role: Role,
    ...
    packs = split_dp_resources(tp_size=tp_size,
                               dp_size=dp_size,
                               npu_pack_max_size=_get_npu_num_per_ray_node())
