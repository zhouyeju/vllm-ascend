...
from vllm.attention.backends.abstract import (AttentionBackend, AttentionImpl,
                                              AttentionLayer, AttentionType, AttentionMetadata)
...
def wait_for_kv_layer_load(layer_name: str, attn_metadata: Union["AttentionMetadata", dict[str, "AttentionMetadata"]]) -> None:
    if not has_kv_transfer_group() or not is_v1_kv_transfer_group() or not attn_metadata:
        return
    kvconnector: KVConnectorBase_V1 = get_kv_transfer_group()
    kvconnector.wait_for_layer_load(layer_name)


def kv_layer_save(layer_name: str, attn_metadata: Union["AttentionMetadata", dict[str, "AttentionMetadata"]], kv_cache_layer: torch.Tensor) -> None:
    if not has_kv_transfer_group() or not is_v1_kv_transfer_group() or not attn_metadata:
        return
    kvconnector: KVConnectorBase_V1 = get_kv_transfer_group()
