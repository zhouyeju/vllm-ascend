...
class RequestMetadata:
    def __init__(self, request_id: str, token_ids: list[int], block_ids: list[int], block_size: int):
        slot_mapping = torch.arange(0, block_size).unsqueeze(0) + torch.tensor(block_ids).unsqueeze(1) * block_size
    ...
    def set_mla(self, is_mla):
        if self.is_mla:
            logger.debug(f"request metatdata reset MLA enabled to {is_mla}")
        self.is_mla = is_mla
...
class ChariotConnectorMetadata(KVConnectorMetadata):    
    ...
    def add_load_request(self, request_id: str, token_ids: list[int], block_ids: list[int], block_size: int):
        self.loading_requests.append(RequestMetadata(request_id, token_ids, block_ids, block_size))

    def add_save_request(self, request_id: str, token_ids: list[int], block_ids: list[int], block_size: int):
        self.saving_requests.append(RequestMetadata(request_id, token_ids, block_ids, block_size))
...
class ChariotFutureWrapper:
    def __init__(self,
                 ...
                 is_mla: bool,
                 timeout: int = CHARIOT_KVCACHE_TIMEOUT // MS_TO_S):
    ...
    def wait_for_load(self, timeout: Optional[int] = None):
        ...
        try:
            ret = self.future.results(timeout)
        except TimeoutError:
            logger.error(f"kvcache with id {self.kvcache_id} wait for load timed out after {timeout} seconds.")
            return
    ...
    def wait_for_save(self, timeout: Optional[int] = None):
        ...
        try:
            ...
            logger.error(f"kvcache with id {self.kvcache_id} wait for save timed out after {timeout} seconds.")
            return
        if ret.status_code == 0:
            return
        if ret.error_message:
...
class ChariotConnector(KVConnectorBase_V1):
    ...
    def extract_kv_from_layer(kv_layer: torch.Tensor,
        ...
        if is_mla:
            ...
            return kv_layer.reshape(2, num_blocks * block_size, -1)[:, slot_mapping, ...]
    ...
    def start_load_kv(self, forward_context: "ForwardContext", **kwargs) -> None:
        ...
        if kvconnector_metadata is None:
            logger.warning("kvconnector calls start_load_kv, but the kvconnector metadata is None, skip")
            return
        ...
        for layer_name in forward_context.no_compile_layers:
            for req_meta in kvconnector_metadata.loading_requests:
                ...
                if seq_len == 0:
                    return
                kv_layer: torch.Tensor = forward_context.no_compile_layers[layer_name].kv_cache[forward_context.virtual_engine]
                kv_dtype = next(iter(kv_layer.values())).dtype if isinstance(kv_layer, dict) else kv_layer[0].dtype
    ...
    def save_kv_layer(self, layer_name: str, kv_layer: torch.Tensor, attn_metadata: "AttentionMetadata", **kwargs) -> None:
        ...
        kvconnector_metadata: ChariotConnectorMetadata = self._get_connector_metadata() # type: ignore
        if kvconnector_metadata is None:
            logger.warning("kvconnector calls save_kv_layer, but the kvconnector metadata is None, skip")
            return
    ...
    def build_connector_meta(self, scheduler_output: SchedulerOutput) -> KVConnectorMetadata:
        ...
        for new_req in scheduler_output.scheduled_new_reqs:
            if new_req.req_id in self.requests_waiting_for_load:
                kvconnector_metadata.add_load_request(request_id=new_req.req_id,
                                                      token_ids=new_req.prompt_token_ids,
                                                      block_ids=new_req.block_ids[0], # only one kvcache group now
                                                      block_size=self.block_size)
            elif self.is_producer:
                kvconnector_metadata.add_save_request(request_id=new_req.req_id,
                                                      token_ids=new_req.prompt_token_ids,
                                                      block_ids=new_req.block_ids[0], # only one kvcache group now
                                                      block_size=self.block_size)
